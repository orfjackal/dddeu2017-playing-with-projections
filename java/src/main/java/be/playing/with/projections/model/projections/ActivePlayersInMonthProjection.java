...
/**
 * Active = has finished at least five games in one month
 */
public class ActivePlayersInMonthProjection implements Projection<Integer> {

  private final YearMonth month;

  public ActivePlayersInMonthProjection(YearMonth month) {
    this.month = month;
  }
  ...
  public Integer project(List<Event> events) {
    ...
    for (Event event : events) {
      ...
      if (event.getType() == EventType.PLAYER_JOINED_GAME) {
  ...
  public String buildResultMessage(Integer projectResult) {
    return projectResult + " active players in " + month;
  }
}
